...
#                    plt.show()
        ...
        with self.test_session() as sess:
            feed_dict, speaker_index = CreateTrainingFeedDict(
                ...
                gc_placeholder, 0, lc_placeholder, local_conditions)
            sess.run(init)
            ...
            if self.generate:
                if self.global_conditioning:
                    ...
                    for (waveform, id) in zip(generated_waveforms, ids):
                        wave_file = '/tmp/{}_{}.wav'.format(
                                        self.id(), id)
                        print("Writing {}".format(wave_file))
                        librosa.output.write_wav(wave_file,
                                                 waveform,
                                                 SAMPLE_RATE_HZ)
                    ...
                    wave_file = '/tmp/{}.wav'.format(self.id())
                    print("Writing {}".format(wave_file))
                    librosa.output.write_wav(wave_file,
